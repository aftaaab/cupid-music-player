...
function PlaylistList({ loading, playlists, loadingPlaylist, onSelect, emptyMessage = 'no playlists found', loadingMessage = 'loading...' }) {
  return (
    <div className="settings-playlist-list">
      {loading ? (
        <div className="settings-label">{loadingMessage}</div>
      ) : playlists.length === 0 ? (
...
export default function App() {
  ...
  // ── User-added songs (mobile file picker) ──────────────
  ...
  const refreshUserSongs = useCallback(async () => {
    if (!hasMobileLibrary) return;
    try {
      const { loadUserPlaylist } = await import('./mobile/library.js');
      setUserSongs(await loadUserPlaylist());
  ...
  const handleRemoveSong = useCallback(async (trackEntry) => {
    if (!window.confirm(t('confirmRemove', { title: trackEntry.title }))) return;
    try {
      const { removeSong } = await import('./mobile/library.js');
      await removeSong(trackEntry.file);
      await refreshUserSongs();
      await loadLocalPlaylist();
    } catch (err) {
      setSettingsError(t('removeSongError', { msg: err.message }));
    }
  }, [refreshUserSongs, loadLocalPlaylist, t]);
  ...
  // Tell the Android layer whether audio is playing so it can keep the
  // WebView alive in the background only when needed (battery-aware).
  useEffect(() => {
  ...
  const loadYoutubePlaylistFromUrl = useCallback(async (rawInput) => {
    ...
    if (!parsed) {
      setSettingsError(t('badYoutubeUrl'));
      return;
    }
    ...
    try {
      ...
      if (tracks.length === 0) {
        setSettingsError(t('playlistEmptyOrPrivate'));
        return;
      }
  ...
  const loadPlaylist = useCallback(async (id, service) => {
    ...
    try {
      ...
      if (tracks.length === 0) {
        setSettingsError(t('playlistEmpty'));
        return;
      }
      setStreamTracks(tracks);
      setSource('streaming');
  ...
  return (
    <div className={`player ${theme !== 'pink' ? `theme-${theme}` : ''}`}>
      <ThemeOverlay theme={theme} isPlaying={player.isPlaying} />
      {/* Base frame */}
      ...
      <div className="now-playing">
        <div className="track-info">
          ...
          <MarqueeText className="track-title" text={track.title === 'No track' ? t('noTrack') : track.title} />
          <div className="track-artist">{t('by')} {track.artist}</div>
        </div>
      </div>
        ...
        <div className="settings-panel">
          <div className="settings-panel-inner">
            ...
            <div className="settings-theme-row">
              {[['en', 'english'], ['es', 'espa\u00f1ol']].map(([value, label]) => (
                <button
                  key={value}
                  className={`settings-theme-btn ${lang === value ? 'active' : ''}`}
            ...
            {musicService === 'youtube' && (
              isYouTubeConfigured() ? (
                !youtubeConnected ? (
                  ...
                  >
                    {youtubeLoggingIn ? t('waitingForBrowser') : t('loginWithGoogle')}
                  </button>
                ) : (
                  <>
                    <PlaylistList
                ...
                <>
                  <input
                    ...
                    type="text"
                    placeholder={t('pastePlaylistLink')}
                  ...
                  >
                    {loadingPlaylist ? t('loading') : t('loadPlaylist')}
                  </button>
                </>
              )
            )}
